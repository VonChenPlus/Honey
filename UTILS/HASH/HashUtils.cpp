...
namespace UTILS
{
    namespace HASH
    {
        ...
        uint32 Fletcher(const uint8 *data_uint8, uint64 length) {
          const uint16 *data = (const uint16 *)data_uint8;
          uint64 len = (length + 1) / 2;
          uint32 sum1 = 0xffff, sum2 = 0xffff;

          while (len) {
            uint64 tlen = len > 360 ? 360 : len;
            len -= tlen;
        ...
        uint32 Adler32(const uint8 *data, uint64 len) {
          uint32 a = 1, b = 0;
          while (len) {
            uint64 tlen = len > 5550 ? 5550 : len;
            len -= tlen;
            do {
              a += *data++;
              b += a;
