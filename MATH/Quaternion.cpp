...
namespace MATH
{
    ...
    void Quaternion::createFromAxisAngle(const Vector3f& axis, float angle, Quaternion* dst) {
        float halfAngle = angle * 0.5f;
        float sinHalfAngle = sinf(halfAngle);

        Vector3f normal(axis);
        normal.normalize();
    ...
    void Quaternion::set(const Vector3f& axis, float angle) {
